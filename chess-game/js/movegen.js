...
function AddEnPassasntMove(move) {
    GameBoard.moveList[  GameBoard.moveListStart[ GameBoard.ply+1 ]  ] = move;
    GameBoard.moveScores[  GameBoard.moveListStart[ GameBoard.ply+1 ]++  ] = 0;
    //GameBoard.moveListStart[ GameBoard.ply+1 ]++;
}
...
function AddWhitePawnQuietMoveMove(from, to) {
    if (RanksBrd[from] == RANKS.RANK_7) {
    ...
    if (GameBoard.side == COLOURS.WHITE) {
        ...
        for (pceNum = 0; pceNum < GameBoard.pceNum[pceType]; ++pceNum) {
            ...
            if (GameBoard.pieces[sq + 10] == PIECES.EMPTY) {
                AddWhitePawnQuietMoveMove(sq, sq + 10);
            ...
            if (GameBoard.enPas != SQUARES.NO_SQ) {
                if (sq + 9 == GameBoard.enPas) {
                    // add en pas move
                    AddEnPassasntMove( MOVE(sq, sq+9, PIECES.EMPTY, PIECES.EMPTY, MFLAGEP));
                }
                if (sq + 11 == GameBoard.enPas) {
                    // add en pas move
                    AddEnPassasntMove( MOVE(sq, sq+11, PIECES.EMPTY, PIECES.EMPTY, MFLAGEP));
                }
            }
        }
        ...
        if (GameBoard.castlePerm && CASTLEBIT.BQCA) {
            if (GameBoard.pieces[SQUARES.D8] == PIECES.EMPTY && GameBoard.pieces[SQUARES.C8] == PIECES.EMPTY && GameBoard.pieces[SQUARES.B8] == PIECES.EMPTY) {
                if (SqAttacked(D8, COLOURS.WHITE) == BOOL.FALSE && SqAttacked(SQUARES.E8, COLOURS.WHITE) == BOOL.FALSE) {
...
function AddWhitePawnMove() {
    if (GameBoard.pieces[to] != PIECES.EMPTY) {
        AddCaptureMove(MOVE(from, to, captured, PIECES.EMPTY, MFLAGPS));
    }
    else {
        AddQuietMove(MOVE(from, to, captured, PIECES.EMPTY, MFLAGPS));
    }
}